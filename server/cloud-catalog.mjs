...
export function isPublicCloudRead(path) {
  return /^\/api\/v1\/(?:tianmo\/(?:brainstorm\/cards(?:\/[^/]+\/image)?|hotboard\/(?:boards|aggregate|search)|scan\/platforms)|tianwang\/catalog(?:\/books)?|tianwang-library\/templates|covers\/file\/asset\/[^/]+)$/.test(path);
}
...
export function cloudCatalog(store, env, access) {
  async function read(path, authenticated = false) {
    await access.requireFree(); const cfg = cloudOptions(store, env);
    let response;
    try { response = await fetch(endpoint(cfg.baseUrl) + '/api/v1' + path, { headers: authenticated ? { Authorization: `Bearer ${cfg.key}`, 'X-Skoob-User': cfg.userId } : {}, redirect: 'error', signal: AbortSignal.timeout(15000) }); }
    ...
    if (now.key !== cfg.key || now.userId !== cfg.userId || now.baseUrl !== cfg.baseUrl) throw new ApiError(409, 'CLOUD_CONNECTION_CHANGED', '账号已改变，请刷新目录。');
    return data;
  }
  ...
  return {
    ...
    async list(collection) {
      ...
      try { return [...local, ...await remote(collection)]; }
      catch (e) { if (['FREE_KEY_REQUIRED', 'FREE_KEY_INVALID', 'FREE_KEY_CHANGED', 'FREE_VERIFY_UNAVAILABLE', 'FREE_VERIFY_FAILED', 'CLOUD_LOGIN_REQUIRED'].includes(e.code)) return local; throw e; }
    },
    async material(collection, id) {
      ...
      if (collection === 'genres') {
        const details = await read('/genres/' + encodeURIComponent(item.officialId) + '/cluster', true);
