...
export async function selectedContext(store, input, material = async (collection, id) => store.get(collection, id)) {
  const items = [];
  for (const id of input.requestedSkills || input.summonedSkillIds || []) {
    const skill = await material('skills', id); if (!skill) throw new ApiError(404, 'SKILL_NOT_FOUND', '所选本地技能不存在'); items.push(JSON.stringify(skill));
  }
  for (const ref of input.capabilityRefs || []) {
    ...
    const item = await material(collection, ref.id); if (!item) throw new ApiError(404, 'ASSET_NOT_FOUND', '所选本地能力不存在'); items.push(JSON.stringify(item));
  }
