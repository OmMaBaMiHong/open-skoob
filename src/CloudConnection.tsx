...
import { useCloudAccess, FREE_RIGHT_LABELS } from "./cloud-access";
...
export function CloudConnection({ onClose, onChanged, initial = false }: { initial?: boolean; onClose: () => void; onChanged: () => Promise<void> }) {
  const currentAccess = useCloudAccess();
  const [cloud,setCloud] = useState<Connection>({ baseUrl:"https://skoob.cc",configured:false });
  const [free,setFree] = useState<{ ready: boolean; plan?: { name: string }; entitlements?: string[]; message?: string; key?: { id: string; name: string }; service?: string }>({ ready: false });
  useEffect(() => { setFree(previous => ({ ...previous, ready: currentAccess.ready, plan: { name: currentAccess.planName }, entitlements: currentAccess.entitlements, message: currentAccess.message })); }, [currentAccess.ready, currentAccess.message, currentAccess.planName, currentAccess.entitlements]);
  ...
  async function refresh() {
    ...
    else { setKeys([]); setKeyId(""); setAccount(null); setAssets([]); setUploaded({}); }
  }
  ...
  async function exchange(code:string,state:string) {
    ...
    await run(async()=>{await request("/exchange","POST",{code,state});setFlow(null);setManualCode("");setUploaded({});await onChanged();await refresh();setNotice("官方账号已连接。可选择已有 Free Key，或直接粘贴官方 Key 查看免费内容。");});
  ...
  return <div className={initial ? "local-entry" : "local-modal"}><section className={`local-panel cloud-panel${initial ? " cloud-entry-panel" : ""}`} role="dialog" aria-modal="true" aria-label="连接 Skoob 官方平台">
    ...
    {initial && <button className="cloud-skip" type="button" disabled={busy} onClick={onClose}>暂不连接，使用自己的模型</button>}
    {free.ready && <section className="cloud-status"><p role="status">官方内容已连接：{free.key?.name || "官方 Key"}。</p><p>{free.plan?.name || "Free 免费套餐"} · 已开放：{free.entitlements?.map(code => FREE_RIGHT_LABELS[code] || code).join("、") || "暂无可用权益"}</p><div className="cloud-connect-methods"><button onClick={onClose}>进入工作室</button><button disabled={busy} onClick={() => void run(async () => { await request("/key", "DELETE"); await onChanged(); await refresh(); setNotice("已断开云端列表，本地作品和模型配置保留。"); })}>断开云端列表</button></div></section>}
    ...
    {account && <section className="cloud-status"><h3>选择官方 Free Key</h3><p>在中转站创建或选择 openskoob-free 分组的有效 Key。这里只显示密钥尾号，验证后保存在本地；不会自动创建或替换你正在使用的模型。</p>
      ...
      {free.ready ? <><p role="status">Free 已接入：{free.key?.name || free.key?.id}，已按当前套餐开放免费内容。</p><div className="cloud-connect-methods"><button onClick={onClose}>查看免费内容</button><a href="/settings/models">选择创作模型</a></div></> : <p>{free.message || "完成 Key 验证后才展示官方云端列表。"}</p>}
    </section>}
    {!initial && <details><summary>使用已有官方登录 Token</summary><form onSubmit={e=>{e.preventDefault();void run(async()=>{await request("","PUT",{baseUrl:cloud.baseUrl,apiKey:key});setKey("");setFlow(null);setUploaded({});await onChanged();await refresh();setNotice("官方身份校验通过，请继续选择 Free Key。");})}}><label>官方访问凭证<input type="password" value={key} onChange={e=>setKey(e.target.value)} autoComplete="off" placeholder="填写官方支持的平台访问凭证" required /></label><p className="local-help">这是平台登录 Token；中转站模型 Key 请填在上方官方 API Key 输入框。</p><button type="submit" disabled={busy}>验证并连接</button></form></details>}
