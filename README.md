...
官方云端能力是独立的付费服务，用户按需要选择。自有模型的费用由用户与模型服务商结算；选择官方中转站，模型 Token 用量由中转站计量和结算。使用自己的模型，不重复向 Skoob 支付这部分模型 Token 费用。
...
2. 在「设置 → 模型配置」添加自己的 OpenAI 兼容服务，填写 Base URL、Key 并选择模型；也可以选 **OpenSkoob 中转站**。
3. 在首页输入灵感，点「以此灵感开始六步创作」。确认意图卡后，按步骤生成、确认或重写，正文保存到本地作品库。普通问题可直接点「发送」进行对话。
4. 点顶部「连接官方平台」，通过官方账号登录授权，或填写官方支持的访问凭证并完成校验。连接后显示账号、套餐和云端资源；四大引擎仍由官方实时校验权益。本地模型 Key 与平台凭证分别配置。
