...
官方脑洞、热点和模板列表在连接有效 Free Key 后免费展示；高级引擎按具体套餐权益提供，用户按需要选择。自有模型的费用由用户与模型服务商结算；选择官方中转站，模型 Token 用量由中转站计量和结算。使用自己的模型，不重复向 Skoob 支付这部分模型 Token 费用。
...
1. 打开 `http://127.0.0.1:9002`，用终端显示的首次访问密码登录。
2. 首次引导推荐「连接官方，领取 Free Key」：复用官方账号授权，在 [中转站密钥页](https://gaotk.com/keys) 创建 `openskoob-free` 分组的 Key，回来刷新并明确选择。已有有效 Free Key 可直接选用；后端校验通过后，首页才展示真实官方脑洞、热点及模板。
   也可选择「使用自己的模型，暂不领取」，在「设置 → 模型配置」添加自己的 OpenAI 兼容服务；本地基础创作不要求购买套餐。
3. 在首页输入灵感，点「以此灵感开始六步创作」。确认意图卡后，按步骤生成、确认或重写，正文保存到本地作品库。普通问题可直接点「发送」进行对话。
4. 点顶部「连接官方平台」，通过官方账号登录授权，或填写官方支持的访问凭证并完成校验。连接后显示账号与套餐；云端列表须再验证 Free Key。Free Key 不等于付费会员，四大引擎仍由官方实时校验权益；选 Key 不会覆盖自己的服务或自动切换模型。
...
Docker 日志中可查看首次密码，数据保存在 `skoob-data` 卷。默认仅开放本机 `9002` 端口。需要远程访问时，自行配置 HTTPS 反向代理；这是个人工作室部署，一个本地密码对应同一份工作区，不是多人隔离平台。

密码可在 `.env` 中设置 `SKOOB_LOCAL_PASSWORD`（至少 10 字符）。本机安装的数据默认位于 `data/`，包含作品、会话、配置和加密凭证；备份时停止服务并完整备份该目录，保留其中的 `secrets.key`。不要上传数据目录或 `.env`。
