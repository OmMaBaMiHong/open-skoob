...
官方脑洞、热点和模板列表在连接有效官方 Key 后，按 **Free 免费套餐的实际权益** 展示；高级引擎按具体套餐权益提供，用户按需要选择。自有模型的费用由用户与模型服务商结算；选择官方中转站，模型 Token 用量由中转站计量和结算。使用自己的模型，不重复向 Skoob 支付这部分模型 Token 费用。
