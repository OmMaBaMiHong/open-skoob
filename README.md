...
> **当前版本调整中：** 当前 `main` 暂为网页版本，本地基础后端尚在恢复。下面的本地／云端划分是完整自部署版的交付目标；当前代码尚不能独立完成本地创作。已完成的恢复与部署验证会随版本更新说明。
...
本地前后端完整部署说明将在后端恢复并验证后补齐。当前分支只能启动网页，环境要求为 Node.js >= 22、npm：
...
页面地址为 `http://127.0.0.1:9002`。当前网页请求官方服务，登录与跨域访问需要对应来源授权；这不是完整自部署版的运行方式。现阶段接入限制见 [接入说明](docs/api-integration.md)。
